...
- The server contract (the load-bearing invariant):
  ...
  protocol doc mandates the invariant, not the mechanism; the backend guide
  (below) sketches known-good mechanisms.
...
- The engine exposes the same surface as upstream (`synchronize()`,
  ...
  chunked internally as an implementation detail, not API).
...
3. **Cursor scope**: one cursor for the whole database vs per-table cursors.
   ...
   extension, not a v1 complication.
