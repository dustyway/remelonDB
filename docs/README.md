...
Working _on_ the library rather than with it? Start with
...
Two kinds of documents live here. **Design decisions** record _why_ the
...
reading them. **Reference guides** describe _what exists and how to use it_;
...
| [codebase-guide.md](codebase-guide.md) | The maintainer guide: the whole codebase cover to cover with each concept taught inline, plus the NotAnotherCards consumer chapter. Renderable to PDF (command in the file header) or EPUB (`pnpm guide:epub`). |
...
| [sync-design.md](sync-design.md)                     | Generic reimplementation of WatermelonDB's sync protocol with two contract-level fixes, an opaque commit-ordered cursor (kills the lost-write race) and push-responds-like-a-pull (kills the push echo), plus a third divergence: per-record rejection as a normative contract (refusals never silent, a rejected id leaves no effect) where upstream has an unnormed experiment.                                                                                                                              |
...
| [zod-adapter.md](zod-adapter.md)                     | Implemented (@remelondb/core/zod): derive client tables from shared Zod schemas (zodTable) and validate both sync directions with wire schemas built from the same objects. No changes to core's main entry; validation lands at the trust boundaries.                                                                                                                                                                                                                                                         |
| [typed-query-values.md](typed-query-values.md)       | Proposed: per-column value typing in Q (`Q.where('position', '5')` becomes a compile error) and `z.enum` columns, via a shared value-type parameter on `ColumnDef`. Phantom types only; runtime, SQL, and wire byte-identical.                                                                                                                                                                                                                                                                                 |
