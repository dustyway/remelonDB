...
| [simplification.md](simplification.md) | Proposed: halve the codebase by parking seam-bounded pieces (C++ RN driver → expo-sqlite wrapper, the in-memory matcher, Q's join layer) with tagged revival paths proven by the conformance suites. Sync, schema/Zod, and the suites themselves stay. |
