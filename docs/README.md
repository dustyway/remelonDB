...
| [simplification.md](simplification.md) | Done: three cuts along seams — expo-sqlite as the default RN driver (C++ module optional), one observation strategy (no in-memory matcher), and the conformance suites plus the Zod adapter folded into core/server as subpaths. Removed code is deleted, not parked; git history is the archive. |
