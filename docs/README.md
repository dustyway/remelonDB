...
| [simplification.md](simplification.md) | Done: three cuts along seams — expo-sqlite as the default RN driver (C++ module optional), one observation strategy (no in-memory matcher), conformance suites folded into core and server as subpaths. Removed code is deleted, not parked; git history is the archive. |
