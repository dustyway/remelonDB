...
describe('schema construction', () => {
  ...
  it('rejects invalid schemas', () => {
    ...
    expect(() => table('tasks', { created_at: c.number().optional() })).toThrow(
      'non-optional number',
    );
    expect(() => appSchema({ version: 0, tables: [] })).toThrow(
      'positive integer',
    );
    expect(() =>
  ...
  it('encodes schema DDL with standard columns and indices', () => {
    const statements = encodeSchema(
      appSchema({ version: 1, tables: [tasksTable] }),
    );
