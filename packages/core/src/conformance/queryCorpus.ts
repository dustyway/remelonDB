...
export function queryCorpusSuite(options: ResolvedOptions): void {
  describe('query semantics on the engine', () => {
    ...
    it('between is inclusive', async () => {
      expect(
        await taskIds([
          Q.where('position', Q.between(2, 4)),
          Q.sortBy('position'),
        ]),
      ).toEqual(['t2', 't3', 't4']);
    });
    ...
    it('like is case-insensitive for ascii letters only', async () => {
      ...
      expect(await taskIds([Q.where('name', Q.like('ORPHAN'))])).toEqual([
        't4',
      ]);
      expect(await taskIds([Q.where('name', Q.like('_rphan'))])).toEqual([
        't4',
      ]);
    ...
    it('includes does literal substring matching', async () => {
      expect(await taskIds([Q.where('name', Q.includes('0%'))])).toEqual([
        't1',
      ]);
    });
    ...
    it('has_many joins deduplicate fanned-out rows, in select and count', async () => {
      expect(
        await taskIds([
          Q.on('comments', 'body', Q.notEq(null)),
          Q.sortBy('position'),
        ]),
      ).toEqual(['t1', 't2']);
      expect(await taskCount([Q.on('comments', 'body', Q.notEq(null))])).toBe(
        2,
      );
    });
