...
const fakeQuery = <M,>(db: Database, table: string, description: unknown) => {
  ...
  const observe = vi.fn(
    (cb: (records: M[]) => void, onError?: (error: Error) => void) => {
      subscribers.add(cb);
  ...
  const observeCount = vi.fn(
    (cb: (count: number) => void, onError?: (error: Error) => void) => {
...
describe('useMutation', () => {
  ...
  it('works when rendered under StrictMode', async () => {
    const { result } = renderHook(
      () => useMutation(async (n: number) => n + 1),
      {
        wrapper: ({ children }) => createElement(StrictMode, null, children),
      },
    );
