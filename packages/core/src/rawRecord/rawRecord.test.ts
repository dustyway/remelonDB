...
describe('sanitizedRaw', () => {
  it('passes valid records through unchanged', () => {
    ...
    expect(sanitizedRaw(dirty, table)).toEqual(dirty)
  })
  ...
  it('generates an id when missing or invalid', () => {
    expect(sanitizedRaw({}, table).id).toMatch(/^[a-z0-9]{16}$/)
    expect(sanitizedRaw({ id: 7 }, table).id).toMatch(/^[a-z0-9]{16}$/)
    expect(sanitizedRaw({ id: '' }, table).id).toMatch(/^[a-z0-9]{16}$/)
    expect(sanitizedRaw({ id: 'keep-me_1' }, table).id).toBe('keep-me_1')
  })

  it('defaults sync fields', () => {
    const raw = sanitizedRaw({ _status: 'exploded', _changed: 42 }, table)
  ...
  it('converts stored 0/1 back to booleans (driver round-trip)', () => {
    expect(sanitizedRaw({ is_done: 1 }, table).is_done).toBe(true)
    expect(sanitizedRaw({ is_done: 0 }, table).is_done).toBe(false)
    expect(sanitizedRaw({ is_done: 2 }, table).is_done).toBe(false)
  })
...
describe('setRawSanitized', () => {
  it('sanitizes single-column writes', () => {
    const raw = sanitizedRaw({}, table)
    setRawSanitized(raw, 'new name', table.columns['name']!)
    expect(raw.name).toBe('new name')
    setRawSanitized(raw, undefined, table.columns['name']!)
    expect(raw.name).toBe('')
    setRawSanitized(raw, 1, table.columns['is_done']!)
