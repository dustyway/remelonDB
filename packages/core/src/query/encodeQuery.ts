...
export function encodeQuery(
  ...
  if (
    ...
    throw new Error(
      'encodeQuery: Q.take/Q.skip are not supported in count mode',
    );
  }
  ...
  const encodeComparison = (
    ...
    switch (operator) {
      ...
      case 'oneOf':
        return `${col} in (${valuesOf(right)
          .map((v) => pushArg(v))
          .join(', ')})`;
      case 'notIn':
        return `${col} not in (${valuesOf(right)
          .map((v) => pushArg(v))
          .join(', ')})`;
      case 'like':
    ...
    switch (clause.type) {
      ...
      case 'or': {
        const parts = clause.conditions.map((c) =>
          encodeCondition(c, tableContext),
        );
  ...
  const hasToManyJoin = usedAssociations.some(
    (a) => a.info.type === 'has_many',
  );
  const select =
    mode === 'count'
      ? hasToManyJoin
