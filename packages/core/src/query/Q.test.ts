...
describe('comparisons', () => {
  it('Q.where with a raw value is shorthand for eq', () => {
    ...
    expect(Q.where('deleted_at', null).comparison.right).toEqual({
      value: null,
    });
  });
  ...
  it('supports column-to-column comparisons', () => {
    expect(
      Q.where('updated_at', Q.gt(Q.column('created_at'))).comparison,
    ).toEqual({
      type: comparisonTag,
      operator: 'gt',
...
describe('condition trees', () => {
  ...
  it('Q.on supports shorthand and condition-list forms', () => {
    ...
    expect(() => Q.on('projects', Q.where('a', 1), Q.eq(2) as never)).toThrow(
      'invalid condition',
    );
