...
describe('encodeQuery', () => {
  ...
  it('compiles between, notIn, like, includes and column comparisons', () => {
    ...
    expect(
      encodeQuery(
        tasks([Q.where('updated_at', Q.gt(Q.column('created_at')))]),
      )[0],
  ...
  it('compiles Q.on to a left join with the deleted filter in the join condition', () => {
    expect(
      encodeQuery(
        tasks([Q.on('projects', 'is_archived', false)], [belongsToProjects]),
      ),
    ).toEqual([
  ...
  it('passes unsafeSqlQuery through with its bound values', () => {
    expect(
      encodeQuery(
        tasks([Q.unsafeSqlQuery('select * from tasks where x = ?', [1])]),
      ),
  ...
  it('rejects invalid combinations', () => {
    ...
    expect(() =>
      encodeQuery(
        tasks([Q.and(Q.on('projects', 'a', 1))], [belongsToProjects]),
      ),
