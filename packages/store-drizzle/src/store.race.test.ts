...
d('pull race on real postgres', () => {
  it('a push committing during a pull is never lost', { timeout: 30_000 }, async () => {
    ...
    try {
      ...
      const racingPull = store.transaction('racer', 'pull', async (tx) => {
        ...
        await gate
        return {
          ids: (await tx.changedSince('tasks', 'racer', since)).map((change) => change.id),
      ...
      accepted(
        await h.push({
          changes: { tasks: { created: [{ id: 'during', name: 'during', done: false }], updated: [], deleted: [] } },
      ...
      const duringRev = Number(
        (await pool.query(`select rev from race_tasks where id = 'during'`)).rows[0].rev,
      )
