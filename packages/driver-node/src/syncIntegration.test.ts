...
describe('sync engine', () => {
  ...
  it('resyncRequired re-pulls from scratch and reconciles (replacement)', async () => {
    ...
    await sync();
    await db.write(() =>
      db.get('tasks').create({ id: 'dirty', name: 'local only' }),
    );
  ...
  describe('fixed flaws (regression tests)', () => {
    it('replacement resync preserves offline deletes (review: confirmed flaw)', async () => {
      server.seed('a', { name: 'a', position: 1 });
      await sync();
    ...
    it('large pulls are chunked past SQLite bound-parameter limits', async () => {
    ...
    it('sparse records from a nonconforming server are rejected, not defaulted', async () => {
    ...
    it('concurrent synchronize() calls coalesce into one run', async () => {
      server.seed('t1', { name: 'x', position: 1 });
