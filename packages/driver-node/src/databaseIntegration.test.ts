...
describe('Database core', () => {
  ...
  describe('CRUD and identity', () => {
    ...
    it('tracks changed columns on synced records', async () => {
      ...
      await db.write(() =>
        db.get('tasks').update('t1', { name: 'b', position: 1 }),
      );
  ...
  describe('atomic delete cascades', () => {
    it('a failed batch rolls back its delete operations too', async () => {
      ...
      await expect(
        db.write(async () =>
          db.batch([
            db
              .get('tasks')
              .prepareMarkAsDeleted(await db.get('tasks').find('child')),
            db
              .get('tasks')
              .prepareMarkAsDeleted(await db.get('tasks').find('parent')),
      ...
      expect((await db.get('tasks').find('parent'))._status).not.toBe(
        'deleted',
      );
  ...
  describe('observation', () => {
    ...
    it('flat queries: membership and content changes emit, others do not', async () => {
      ...
      unsubscribe();
      await db.write(() =>
        db.get('tasks').create({ id: 't3', is_done: false }),
      );
      await flush();
      expect(emissions).toHaveLength(4);
    });
    ...
    it('join queries reload when the joined table changes', async () => {
      ...
      await db.write(() =>
        db.get('projects').update('p1', { name: 'renamed' }),
      );
      await flush();
