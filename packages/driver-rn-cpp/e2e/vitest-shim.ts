...
export function beforeAll(fn: TestFn): void {
  current.beforeAll.push(fn);
}
export function afterAll(fn: TestFn): void {
  current.afterAll.push(fn);
}
...
function isEqual(a: unknown, b: unknown): boolean {
  ...
  const ka = Object.keys(a as object).filter(
    (k) => (a as any)[k] !== undefined,
  );
  const kb = Object.keys(b as object).filter(
    (k) => (b as any)[k] !== undefined,
  );
...
export function expect(actual: unknown) {
  const matchers = (negate: boolean) => {
    ...
    return {
      toBe: (e: unknown) =>
        check(Object.is(actual, e), `expected ${show(actual)} toBe ${show(e)}`),
      toEqual: (e: unknown) =>
        check(
          isEqual(actual, e),
          `expected ${show(actual)} toEqual ${show(e)}`,
        ),
      toBeNull: () =>
        check(actual === null, `expected ${show(actual)} toBeNull`),
        ...
        const matches =
          ...
          (pattern == null ||
            (pattern instanceof RegExp
              ? pattern.test(String(threw))
              : String(threw).includes(pattern)));
        check(
          Boolean(matches),
          didThrow
        ...
        if (pattern != null) {
          const ok =
            pattern instanceof RegExp
              ...
              : String(threw).includes(pattern);
          if (!ok)
            fail(`rejection ${show(String(threw))} does not match ${pattern}`);
        }
      },
    },
  };
...
export async function runRegisteredSuites(
  ...
  async function runSuite(
    suite: Suite,
    path: string[],
    inheritedEach: { before: TestFn[]; after: TestFn[] },
  ): Promise<void> {
    const each = {
