/**
 * Runtime smoke test for @remelondb/driver-rn-cpp.
...
async function runConformance(push: (r: Result) => void): Promise<void> {
  ...
  console.log(
    `WMCONF: ${report.passed} passed, ${report.failed.length} failed`,
  );
  push({
...
export default function App(): React.JSX.Element {
  ...
  const [verdict, setVerdict] = useState<'RUNNING' | 'PASS' | 'FAIL'>(
    'RUNNING',
  );
  ...
  useEffect(() => {
    const push = (r: Result) => {
      console.log(
        `WMSMOKE: ${r.ok ? 'ok' : 'FAIL'} - ${r.name}${r.detail ? ` :: ${r.detail}` : ''}`,
      );
