...
async function secondRun(
  ...
  push({
    name: 'reopen after reload (previous connection left open)',
    ok: true,
  });
  ...
  await step(
    'writes work in the new instance (no lingering lock)',
    async () => {
      await d.execute('INSERT INTO t (id, n) VALUES (?, ?)', [
        'after-reload',
        2,
      ]);
      const rows = await d.query('SELECT COUNT(*) AS c FROM t', []);
...
export default function App(): React.JSX.Element {
  ...
  const [verdict, setVerdict] = useState<
    'RUNNING' | 'RELOADING…' | 'RELOAD PASS' | 'RELOAD FAIL'
  >('RUNNING');

  useEffect(() => {
    const push = (r: Result) => {
      console.log(
        `WMRELOAD: ${r.ok ? 'ok' : 'FAIL'} - ${r.name}${r.detail ? ` :: ${r.detail}` : ''}`,
      );
