...
export type {
  ReferenceServer,
  ReferenceServerOptions,
  SyncHandlers,
} from './referenceServer';
  ...
  describe(`sync server conformance: ${options.name}`, () => {
    ...
    it('2. incremental pull returns exactly the rows changed after the cursor', async () => {
      ...
      const afterFirst = accepted(
        await handlers.push({
          changes: changesWith([first]),
          cursor: start.cursor,
        }),
      );
      const caughtUp = pulled(
        await pullFrom(handlers, afterFirst.cursor ?? start.cursor),
      );

      const second = fixture.validRow();
      accepted(
        await handlers.push({
          changes: changesWith([second]),
