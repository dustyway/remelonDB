...
for (const path of globSync("packages/*/package.json").sort()) {
  ...
  for (const [subpath, src] of Object.entries(pkg.exports)) {
    ...
    if (!m) {
      console.error(`${path}: exports["${subpath}"] = "${src}" does not match ./src/**/*.ts`);
      process.exit(1);
    }
    const dist = `./dist/${m[1]}`;
  ...
  if (before !== after) {
    drift += 1;
    if (check) {
      console.error(`${path}: publishConfig.exports is out of sync with exports`);
    } else {
...
console.log(check ? "publishConfig.exports in sync" : `done (${drift} file(s) changed)`);
