...
const doc = await readFile(
  new URL('../docs/sync-tour.md', import.meta.url),
  'utf8',
);
const blocks = [];
let fragments = 0;
...
if (blocks.length < 2)
  throw new Error('no json request/response pairs found in the tour');
...
const parsed = blocks.map((b, i) => {
  try {
    ...
    throw new Error(
      `tour block ${i + 1} is not valid JSON (${error.message}):\n${b}`,
    );
  }
});
...
try {
  ...
  for (const [index, [request, expected]] of pairs.entries()) {
    ...
    if (response.status !== 200) {
      throw new Error(
        `stop ${index + 1}: HTTP ${response.status}, expected 200`,
      );
    }
  ...
  if (expect400) {
    ...
    if (response.status !== 400) {
      throw new Error(`validation stop: HTTP ${response.status}, expected 400`);
    }
  }
